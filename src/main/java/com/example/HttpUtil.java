...
public class HttpUtil {
    ...
    public static String postDataString(String url, String paramMap) {
        ...
        try (CloseableHttpClient closeableHttpClient = httpClientBuilder.build()) {
            // HttpEntity entity = new StringEntity(jsonStrData);
            // 修复 POST json 导致中文乱码
            HttpEntity entity = new StringEntity(paramMap, "UTF-8");
            post.setEntity(entity);
            post.setHeader("Content-type", "application/json");
            HttpResponse resp = closeableHttpClient.execute(post);
            try {
    ...
    public static void main(String[] args) {
        String url = "http://localhost:8888/s/users?pageNum=1&pageSize=10&order=\"desc\"&sort=\"age\"&filter= \"{\\\"sex\\\":1}\"";
...
//        String postResult = HttpUtil.postData(url, json.toJSONString());
...
//        System.out.println(formResult);
    ...
    public static MyHttpResponse postDataWithStatusCode(String url, Map<String, Object> paramMap) {
        CloseableHttpClient httpClient = HttpClientBuilder.create().build();
        HttpPost httpPost = new HttpPost(url);

        try {

            // 1. 将参数Map序列化为JSON字符串（需确保paramMap为合法JSON结构）
            String jsonBody = new JSONObject(paramMap).toJSONString(); // 使用JSON库（如FastJSON）
            ...
            // 4. 返回成功响应
            return new MyHttpResponse(statusCode, body);

        } catch (Exception e) {
            // 5. 返回错误响应
            return new MyHttpResponse(500, "请求失败: " + e.getMessage(), true);
        } finally {
            try {
                httpClient.close();
            } catch (IOException e) {
